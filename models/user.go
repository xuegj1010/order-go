...
type User struct {
	...
	UpdatedTime time.Time `gorm:"type:timestamp; not null;autoUpdateTime;comment:最后一次更新时间"`
	CreatedTime time.Time `gorm:"type:timestamp; not null;autoCreateTime;comment:插入时间"`
}
